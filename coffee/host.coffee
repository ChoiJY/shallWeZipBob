...
# first, get the data in the right format
      ...
      node = svgEnter.append("g").attr("id", "bubble-nodes1")
        .attr("transform", "translate(#{margin.left},#{margin.top})")

      # clickable background rect to clear the current selection
      node.append("rect")
        .attr("id", "bubble-background1")
        .attr("width", width)
        .attr("height", height)
        .on("click", clear)
      ...
      #  the bubbles in svg
      label = d3.select(this).selectAll("#bubble-labels1").data([data])
        .enter()
        .append("div")
        .attr("id", "bubble-labels1")
...
# idValue returns
    node = node.selectAll(".bubble-node1").data(data, (d) -> idValue(d))
...
# point is
    label = label.selectAll(".bubble-label1").data(data, (d) -> idValue(d))
    ...
    labelEnter = label.enter().append("a")
      .attr("class", "bubble-label1")
      .attr("href", (d) -> "##{encodeURIComponent(idValue(d))}")
      .call(force.drag)
      .call(connectEvents)

    labelEnter.append("div")
      .attr("class", "bubble-label-name1")
      .text((d) -> textValue(d))

    labelEnter.append("div")
      .attr("class", "bubble-label-value1")
      .text((d) -> rValue(d))
...
# scale the distance based on the jitter variable
  ...
  # ---
  updateActive = (id) ->
    node.classed("bubble-selected1", (d) -> id == idValue(d))
  ...
  # ---
  mouseover = (d) ->
    node.classed("bubble-hover1", (p) -> p == d)

  # ---
  # remove hover class
  # ---
  mouseout = (d) ->
    node.classed("bubble-hover1", false)
...
texts = [
  {key:"sherlock",file:"host.csv",name:"The Adventures of Sherlock Holmes"}
]
...
# create a new Bubbles chart
  ...
  $("#text-select1").val(key)


  # set the book title from the text name
  d3.select("#book-title1").html(text.name)
